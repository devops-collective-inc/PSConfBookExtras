...
{
    [Cmdlet(VerbsData.ConvertTo, "Fahrenheit", DefaultParameterSetName = "Celsius")]
    public class ConvertToFahrenheitCommand : PSCmdlet
    {
        [Parameter(
            Mandatory = true,
            Position = 0,
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("ToText"))
            {
                outputString = MyInvocation.BoundParameters.ContainsKey("Precision") ?
                    temperature.ConvertToString(TemperatureUnit.F, (int)Precision) :
                    temperature.ConvertToString(TemperatureUnit.F);
            }
            else
            {
                outputDecimal = MyInvocation.BoundParameters.ContainsKey("Precision") ?
                    temperature.ConvertTo(TemperatureUnit.F, (int)Precision) :
                    temperature.ConvertTo(TemperatureUnit.F);
            }
