...
function Get-VICredentialStoreItem
{
    ...
    begin
    {
        try
        {
            ...
            {
                $cmdletName = 'PCLIProxies-v2\Get-VICredentialStoreItemCore'
            }
        ...
        if (Test-Path -Path $File)
        {
            ...
            ForEach-Object -Process {
                New-Object -TypeName PSObject -Property @{
                    ...
                    User = $User
                    Password = {
                        $ssPswd = $t | ConvertTo-SecureString -Key $key[0..31]
                        $cred = New-Object -TypeName PSCredential 'dummy', $ssPswd
...
function New-VICredentialStoreItem
{
    ...
    begin
    {
        try
        {
            ...
            if ($PSEdition -eq 'Core')
            {
                $cmdletName = 'PCLIProxies-v2\New-VICredentialStoreItemCore'
            }
            $cmdletType = [System.Management.Automation.CommandTypes]::Cmdlet
            $wrappedCmd = $ExecutionContext.InvokeCommand.GetCommand($cmdletName, $cmdletType)
            $scriptCmd = { & $wrappedCmd @PSBoundParameters }
...
function New-VICredentialStoreItemCore
{
    ...
    {
        $txt = openssl genrsa 512
        $txt[1..($txt.Count - 2)] | Set-Content -Path $keyFile
        chmod 600 $keyfile
    }
    $key = [System.IO.File]::ReadAllBytes($keyFile)
    ...
    New-Object -TypeName PSObject -Property @{
        ...
        User = $User
        Password = {
            $ssPswd = $Password | ConvertTo-SecureString -AsPlainText -Force
            $ssPswd | ConvertFrom-SecureString -Key $key[0..31]
        }
...
function Remove-VICredentialStoreItem
{
    ...
    begin
    {
        try
        {
            ...
            if ($PSEdition -eq 'Core')
            {
                $cmdletName = 'PCLIProxies-v2\Remove-VICredentialStoreItemCore'
            }
            $cmdletType = [System.Management.Automation.CommandTypes]::Cmdlet
            $wrappedCmd = $ExecutionContext.InvokeCommand.GetCommand($cmdletName, $cmdletType)
            $scriptCmd = { & $wrappedCmd @PSBoundParameters }
